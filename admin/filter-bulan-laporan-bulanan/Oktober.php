...
$laporanKeuangan = query("SELECT * FROM t_lap_keuangan WHERE MONTHNAME(tanggal)='October' ORDER BY id_lap_keuangan DESC");



$queryPemasukan = query(
    "SELECT SUM(nominal) AS total_nominal 
                 FROM t_lap_keuangan WHERE status = 0 AND MONTHNAME(tanggal)='October'"
);
$queryPengeluaran = query(
    "SELECT SUM(nominal) AS total_nominal 
                 FROM t_lap_keuangan WHERE status = 1 AND MONTHNAME(tanggal)='October'"
);
